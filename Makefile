...
usr/local/lib/libcal3d.la : ${PWD}/usr/local
	mkdir -p usr/local && tar -zvxf cal3d-0.11.0.tar.gz && patch -p3 < cal3d.patch &&  cd cal3d-0.11.0 && ./configure --prefix ${PWD}/usr/local && echo "#include <cstring>" >> config.h && echo "#include <memory>" >> config.h && make clean && make && make install && touch cal3d
